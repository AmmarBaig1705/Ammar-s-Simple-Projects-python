...
AdminFile = 'AdminInfo.json'

def UpdateAdminFile(): #Method to Update Admin Accounts File
    with open(AdminFile, 'w') as f:
        json.dump(AdminAccounts, f,  indent=4)


def LoadAdminFile(): #Method of loading Admin Accounts in a Dictionary
    global AdminAccounts
    if os.path.exists(AdminFile):
        with open(AdminFile, 'r') as f:
            AdminAccounts = json.load(f)
    else:
        AdminAccounts = {}
...
def RegistrationPage(): #User Registration Page
    ...
    while len(UserNameInput) > 15 :
        UserNameInput = input("Invalid Range! Enter Username below 15 characters : ")
...
def Introduction(): #Introduction Page for User/Admin
    ...
    print("0 - Login")
    print("1 - Register")
    print("* - Exit Banking Page")
    UserChoice = str(input("Enter Choice: "))
    while UserChoice != "0" and UserChoice != "1" and UserChoice != "*":  #Checks for valid Value 0, 1 or *
    ...
    if UserChoice == "0":
        ...
        print("----------------------------------------")
        print("Thank you for Visiting Pacific Bank!")
...
def AdminMenu(): #Shows Admin their menu
    ...
    while AdminChoice != "0" and AdminChoice != "1" and AdminChoice != "2" and AdminChoice != "3" and AdminChoice != "4" and AdminChoice != "5":
    ...
    if AdminChoice == "0":
        ...
        AdminAddsNewAdmin()
    elif AdminChoice == "5":  #Admin Logging out
        print("Logging out...")
        print("----------------------------------")
        print("")
        Introduction()
...
def AdminUpdateOwnPassword(): #3 - Admin Changing His Own Password
    OldPassword = AdminAccounts[UserID]['Password']
    ...
    if OldPasswordConfirm == OldPassword:
        ...
        AdminAccounts[UserID]['Password'] = NewPassword
        print("--------------------------------------")
    ...
    print("Returning to Admin Menu...")
    AdminMenu()
def AdminAddsNewAdmin(): #4 - Admin Adding a new Admin
    NewUserID = f"{int(max(AdminAccounts.keys(), default='90000')) + 1:05d}" #Generate a New UserID
    ...
    UpdateAdminFile() #Updating Dictionary for newly added admin
    print("")
    print(f"User {UserNameInput} registered successfully with ID: {NewUserID}")
    print("Thank you for Registering! You may now login.")
    print("Returning to Admin Menu....")
    ...
    AdminMenu() #Returns User back to Start menu
